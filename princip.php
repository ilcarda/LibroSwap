...
if(isset($_POST['login'])) {
    ...
    if($result->num_rows === 1) {
        ...
        // ★ MODIFICA: usa password_verify() per confrontare la password inserita con l'hash
...
if(isset($_POST['register'])) {
    ...
    $email = strtolower(trim($_POST['email_reg']));
    $password = $_POST['password_reg'];
    $confirm = $_POST['confirm_password'];

    // Validazioni
    if(empty($nome) || empty($cognome) || empty($email) || empty($password) || empty($confirm)) {
        $erroreRegister = "Compila tutti i campi";
    }
    elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $erroreRegister = "Email non valida";
    }
    elseif(strlen($password) < 6) {
        $erroreRegister = "La password deve avere almeno 6 caratteri";
    }
    elseif($password !== $confirm) {
        $erroreRegister = "Le password non coincidono";
    }
    else {
        // Controllo se email esiste già
        $check = $conn->prepare("SELECT id FROM utente WHERE LOWER(email) = ?");
        $check->bind_param("s", $email);
        $check->execute();
        $result = $check->get_result();

        if($result->num_rows > 0) {
            $erroreRegister = "Email già registrata";
        } else {
            $passwordHash = password_hash($password, PASSWORD_DEFAULT);
            $ruolo = "STUDENTE"; // o il ruolo che preferisci

            $stmt = $conn->prepare("INSERT INTO utente (nome, cognome, email, password, ruolo) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $nome, $cognome, $email, $passwordHash, $ruolo);

            if($stmt->execute()) {
                // Login automatico (stessa sessione, senza rigenerare l'ID)
                ...
                $erroreRegister = "Errore durante la registrazione. Riprova.";
            }
        }
    }
}
...
<body>
    <!-- Overlay di login -->
    <!-- Overlay di login/registrazione -->
<div id="loginOverlay" class="login-overlay" style="<?php echo isset($_SESSION['user_id']) ? 'display:none;' : 'display:flex;'; ?>">
    <div class="login-container">
        <div class="login-header">
            <h2 id="overlayTitle">BookSwap</h2>
            <p id="overlaySub">Accedi al tuo account per continuare</p>
        </div>

        <!-- Messaggi di errore -->
        <?php if(isset($erroreLogin) && $erroreLogin): ?>
            <div class="alert alert-danger"><?php echo htmlspecialchars($erroreLogin); ?></div>
        <?php endif; ?>
        <?php if(isset($erroreRegister) && $erroreRegister): ?>
            <div class="alert alert-danger"><?php echo htmlspecialchars($erroreRegister); ?></div>
        <?php endif; ?>

        <!-- Tabs per switch -->
        <div class="login-tabs" style="display:flex; gap:8px; margin-bottom:20px;">
            <button id="tabBtnLogin" class="btn" style="flex:1; background:#3498db;">Accedi</button>
            <button id="tabBtnRegister" class="btn" style="flex:1; background:#27ae60;">Registrati</button>
        </div>

        <!-- FORM LOGIN (inizialmente visibile) -->
        ...
        <!-- FORM REGISTRAZIONE (inizialmente nascosto) -->
        <form id="registerForm" method="POST" style="display:none;">
            <div class="form-group">
                <label for="nome">Nome</label>
                <input type="text" name="nome" id="regNome" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="cognome">Cognome</label>
                <input type="text" name="cognome" id="regCognome" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="email_reg">Email</label>
                <input type="email" name="email_reg" id="regEmail" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="password_reg">Password (min. 6 caratteri)</label>
                <input type="password" name="password_reg" id="regPassword" class="form-control" required>
            </div>
            <div class="form-group">
    ...
    <script>
        ...
        // Gestione toggle tra login e registrazione nell'overlay
(function() {
    ...
    function showLogin() {
        ...
        overlayTitle.textContent = 'BookSwap';
        overlaySub.textContent = 'Accedi al tuo account per continuare';
    ...
    function showRegister() {
        ...
        tabLogin.style.background = '#95a5a6';
        tabRegister.style.background = '#27ae60';
        overlayTitle.textContent = 'Crea un account';
        overlaySub.textContent = 'Registrati per iniziare a usare BookSwap';
        // Focus sul primo campo
    ...
    // Se c'è un errore di registrazione, mostra automaticamente il form di registrazione
    <?php if(isset($erroreRegister) && $erroreRegister): ?>
    showRegister();
    <?php else: ?>
    showLogin();
